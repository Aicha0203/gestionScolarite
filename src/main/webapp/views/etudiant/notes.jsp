...
<%@ page
	import="jakarta.servlet.http.HttpSession, java.util.List, models.Note"%>
...
if (!notes.isEmpty()) {
	...
	for (Note note : notes) {
		int credit = (note.getCredit() > 0) ? note.getCredit() : 1;
		double valeur = (note.getMoyenne() >= 0) ? note.getMoyenne() : 0;
...
<link rel="stylesheet"
	href="<%=request.getContextPath()%>/assets/css/note.css">
<link rel="stylesheet"
...
<body>
	<div class="layout">
		...
		<aside class="sidebar">
			...
			<nav class="menu">
				<ul>
					<li><a href="<%=request.getContextPath()%>/EtudiantServlet"><i
							class="fas fa-th-large"></i> Tableau de bord</a></li>
					<li><a
						href="<%=request.getContextPath()%>/CoursServlet?programmeId=1"><i
							class="fas fa-book"></i> Cours</a></li>
					<li><a
						href="<%=request.getContextPath()%>/views/etudiant/emploi_du_temps.jsp"><i
							class="fas fa-calendar"></i> Emploi du temps</a></li>
					<li><a
						href="<%=request.getContextPath()%>/FactureServlet?etudiantId=<%=session.getAttribute("userId")%>"><i
							class="fas fa-wallet"></i> Finances</a></li>
					<li class="active"><a
						href="<%=request.getContextPath()%>/NoteServlet?etudiantId=<%=session.getAttribute("userId")%>"><i
							class="fas fa-chart-bar"></i> Notes</a></li>
					<li><a href="<%=request.getContextPath()%>/ProfilServlet">
							<i class="fas fa-user"></i> Profil
					</a></li>
				</ul>
				<div class="logout">
					<a href="<%=request.getContextPath()%>/deconnexion"><i
					...
					<div class="table-wrapper">
						<table class="results-table">
							...
							<tbody>
								...
								%>
								<tr>
									<td colspan="6" class="empty-results">Aucune note
								...
								for (Note note : notes) {
								%>
								<tr class="<%=note.getMoyenne() >= 10 ? "passed" : "failed"%>">
									<td><%=(note.getCodeMatiere() != null && !note.getCodeMatiere().isEmpty()) ? note.getCodeMatiere() : "N/A"%></td>
									<td><%=(note.getNomMatiere() != null && !note.getNomMatiere().isEmpty()) ? note.getNomMatiere() : "N/A"%></td>
									<td class="note-value"><%=String.format("%.2f", note.getMoyenne())%></td>
									<td><%=(note.getCredit() > 0) ? note.getCredit() : "-"%></td>
									<td class="status"><span
										class="status-badge <%=note.getMoyenne() >= 10 ? "success" : "danger"%>">
											<%=note.getMoyenne() >= 10 ? "Réussi" : "Échoué"%>
									</span></td>
									<td><%=(note.getSession() != null && !note.getSession().isEmpty()) ? note.getSession() : "Normale"%></td>
								</tr>
								<%
								}
								%>
				...
				<div class="results-info-panel">
					<div class="academic-summary">
						...
						<div class="progress-container">
							<div class="progress-label">
								<span>Progression du Diplôme</span> <span><%=totalReussis%>/180
									crédits</span>
							</div>
							<div class="progress-bar">
								<div class="progress-fill"
									style="width: <%=Math.min(100, totalReussis * 100 / 180)%>%;"></div>
							</div>
						</div>
					</div>
					...
					<div class="academic-services">
						...
						<div class="services-list">
							<a href="#" class="service-item"> <i class="fas fa-file-alt"></i>
								<span>Demander une attestation</span>
							</a> <a href="#" class="service-item"> <i
								class="fas fa-exclamation-circle"></i> <span>Contestation
									de note</span>
							</a> <a href="#" class="service-item"> <i
								class="fas fa-user-graduate"></i> <span>Consulter un
									conseiller</span>
							</a> <a href="#" class="service-item"> <i
								class="fas fa-calendar-check"></i> <span>Planifier un
									rendez-vous</span>
							</a>
						</div>
